...
fn benchmark_exclusive_partition<M>(channel: &Channel<M>, partition_id: usize, chunk: usize, amount: usize) {
...
fn benchmark_concurrent_partition<M>(channel: &Channel<M>, partition_id: usize, chunk: usize, amount: usize) {
    let mut partition = channel
    ...
    c.bench_with_input(
        ...
        |b, args | {
            b.iter_custom(|iterations| {
                ...
                let thread_one = std::thread::spawn(move || {
                    benchmark_concurrent_partition(&channel, 0, chunk_size, iterations as usize)
                });
