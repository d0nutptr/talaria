...
//!
//! #### Partitions
//! Partitions can be thought of as "states" for the data managed by the channel. Why not
//! consider partitions as "owners" of data? Partitions can be configured to either be
//! [Exclusive](partition::Exclusive) or [Concurrent](partition::Concurrent), which describes
//! whether or not multiple instances of the partition can be held simultaneously. So, when all
//! partitions are in "exclusive" mode, it might make sense to consider them as distinct "owners",
//! but concurrent access to a partition changes this somewhat.
//!
...
//!
//! Creating a channel can be done via Channel's [builder](channel::Channel::builder) method. We'll
//! need to pass in a fixed collection of objects for it to manage when we call
//! [build](channel::Builder::build).
//!
//! Afterwards, depending on how we configured our partitions, we'll either call
//! [get_exclusive_partition](channel::Channel::get_exclusive_partition)
//! or [get_concurrent_partition](channel::Channel::get_concurrent_partition) to get a partition.
//! This will return a [partition](partition::Partition) which we can use to acquire reservations.
//!
//! Finally, we can call [reserve](partition::Partition::reserve) on the partition to get a
//! reservation. Once we hold a reservation, we can either index directly into the reservation or
//! call [iter](partition::Reservation::iter) (or [iter_mut](partition::Reservation::iter_mut))
