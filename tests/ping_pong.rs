...
fn exclusive_blocking_ping_pong() {
    ...
    fn run(channel: Channel<Message>, partition_id: usize, amount: usize, expectation: Message) {
        ...
        for _ in 0 .. amount {
            ...
            assert_eq!(reservation[0], expectation, "Thread expected a {expectation:#?}");
    ...
    get_loom_testing_model(None).check(|| {
        ...
        let ping_handle = loom::thread::spawn(move || run(ping_channel, 0, MESSAGE_COUNT, Message::Pong));
        let pong_handle = loom::thread::spawn(move || run(pong_channel, 1, MESSAGE_COUNT, Message::Ping));
...
fn concurrent_blocking_ping_pong() {
    ...
    get_loom_testing_model(None).check(|| {
        ...
        let ping_one_handle = loom::thread::spawn(move || run(ping_one_channel, 0, MESSAGE_COUNT, Message::Pong));
        let ping_two_handle = loom::thread::spawn(move || run(ping_two_channel, 0, MESSAGE_COUNT, Message::Pong));
        let pong_handle = loom::thread::spawn(move || run(pong_channel, 1, MESSAGE_COUNT * 2, Message::Ping));
