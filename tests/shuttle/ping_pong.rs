...
fn concurrent_blocking_ping_pong() {
    ...
    shuttle::check_random(|| {
        ...
        let ping_one_handle = shuttle::thread::spawn(move || run(ping_one_channel, 0, MESSAGE_COUNT, Message::Pong));
        let ping_two_handle = shuttle::thread::spawn(move || run(ping_two_channel, 0, MESSAGE_COUNT, Message::Pong));
        let pong_handle = shuttle::thread::spawn(move || run(pong_channel, 1, MESSAGE_COUNT * 2, Message::Ping));
